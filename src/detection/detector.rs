...
impl MeetingDetector {
    ...
    pub async fn on_window_change(&mut self, window: WindowInfo) {
        ...
        match (&self.state, meeting_app) {
            ...
            (DetectionState::Idle, Some(app)) => {
                ...
                let _ = self
                    .event_tx
                    .send(DetectorEvent::MeetingDetected { app, window })
                    .await;
            }
            ...
            (DetectionState::Detected { app, .. }, None) => {
                ...
                let _ = self
                    .event_tx
                    .send(DetectorEvent::MeetingEnded { app })
                    .await;
            }
            ...
            (DetectionState::Recording { app, since }, Some(new_app)) if *app == new_app => {
                ...
                self.state = DetectionState::Recording {
                    app: *app,
                    since: *since,
                };
            }
            ...
            (DetectionState::Recording { app, .. }, None) => {
                ...
                let _ = self
                    .event_tx
                    .send(DetectorEvent::RecordingStopped { app })
                    .await;
                let _ = self
                    .event_tx
                    .send(DetectorEvent::MeetingEnded { app })
    ...
    pub async fn start_recording(&mut self) {
        if let DetectionState::Detected { app, .. } = self.state {
            ...
            let _ = self
                .event_tx
                .send(DetectorEvent::RecordingStarted { app })
                .await;
        }
    }
...
mod tests {
    ...
    async fn test_start_recording() {
        ...
        assert!(matches!(
            event,
            DetectorEvent::RecordingStarted {
                app: MeetingApp::Zoom
            }
        ));
    ...
    async fn test_stop_recording() {
        ...
        assert!(matches!(
            event,
            DetectorEvent::RecordingStopped {
                app: MeetingApp::Zoom
            }
        ));
        assert!(!detector.is_recording());
